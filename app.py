...
def main():
    ...
    # ✅ 탭 상태 유지를 위한 콜백 함수들
    def set_tab_new_request():
        st.session_state.current_tab = "새 면접 요청"
    
    def set_tab_progress():
    ...
    # ✅ 탭 전환 감지 및 상태 업데이트
    # JavaScript를 통한 탭 전환 감지는 복잡하므로, 
    # 대신 각 탭의 시작 부분에서 현재 탭 상태를 업데이트하도록 수정
    
    # 현재 활성화된 탭 감지를 위한 숨겨진 버튼들
    if st.button("", key="tab1_detector", help="새 면접 요청 탭"):
        st.session_state.current_tab = "새 면접 요청"
    
    if st.button("", key="tab2_detector", help="진행 현황 탭"):
        st.session_state.current_tab = "진행 현황"
